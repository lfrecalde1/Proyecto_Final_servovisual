...
%plot(t,ul,'-','Color',C2,'LineWidth',lw);
grid minor;
axis([-0.5 0.5 -0.5 0.5 -0.05 3])

subplot(2,1,2)
xlim([-uo uo])
ylim([-vo vo])
set(gca,'Ydir','reverse')
plot(uv_ibvs(1,:)-50,uv_ibvs(2,:),'--','Color',C19,'LineWidth',lw*1); hold on
plot(uv_ibvs(3,:)-50,uv_ibvs(4,:),'--','Color',C21,'LineWidth',lw*1);
plot(uv_ibvs(5,:)-50,uv_ibvs(6,:),'--','Color',C23,'LineWidth',lw*1); hold on
plot(uv_ibvs(7,:)-50,uv_ibvs(8,:),'--','Color',C25,'LineWidth',lw*1);

plot(uv_d_ibvs(1,1)-50,uv_d_ibvs(2,1),'o','Color',C9,'LineWidth',lw*1.5);
plot(uv_d_ibvs(3,1)-50,uv_d_ibvs(4,1),'o','Color',C9,'LineWidth',lw*1.5);
plot(uv_d_ibvs(5,1)-50,uv_d_ibvs(6,1),'o','Color',C9,'LineWidth',lw*1.5);
plot(uv_d_ibvs(7,1)-50,uv_d_ibvs(8,1),'o','Color',C9,'LineWidth',lw*1.5);
%plot(t,ul,'-','Color',C2,'LineWidth',lw);
grid minor;
plot([-uo+20 -uo+20],[-vo+20 vo-20],'k--','Linewidth',lwV);
plot([uo-20 uo-20],[-vo+20 vo-20],'k--','Linewidth',lwV);
plot([-uo+20 uo-20],[-vo+20 -vo+20],'k--','Linewidth',lwV);
plot([-uo+20 uo-20],[vo-20 vo-20],'k--','Linewidth',lwV);
text(-70,150,'$\textrm{Features outside the FOV}$','interpreter','latex','fontsize',12);
set(gca,'ticklabelinterpreter','latex',...
    'fontsize',fontsizeTicks)
...
set(gcf, 'PaperPositionMode', 'manual');
box on
he1_ibvs(1,105:end) = he1_ibvs(1,105);
he2_ibvs(1,105:end) = he2_ibvs(1,105);
he3_ibvs(1,105:end) = he3_ibvs(1,105);
he4_ibvs(1,105:end) = he4_ibvs(1,105);
...
plot([10 10],[0 120],'k--','Linewidth',lwV);
area([0 10],[120 120],'Facecolor','k','Facealpha',0.05);
xlim([0 89])


grid minor;
text(12,110,'$\textrm{Features outside the FOV}$','interpreter','latex','fontsize',12);
set(gca,'ticklabelinterpreter','latex',...
        'fontsize',fontsizeTicks)
%xlabel('$\textrm{Time}[s]$','interpreter','latex','fontsize',fontsizeLabel)
...
plot([10 10],[0 0.2],'k--','Linewidth',lwV);
plot([10 10],[0 -0.5],'k--','Linewidth',lwV);
area([0 10],[0.2 0.2],'Facecolor','k','Facealpha',0.05);
area([0 10],[-0.5 -0.5],'Facecolor','k','Facealpha',0.05);
xlim([0 89])

grid minor;
text(12,0.1,'$\textrm{Features outside the FOV}$','interpreter','latex','fontsize',12);
set(gca,'ticklabelinterpreter','latex',...
        'fontsize',fontsizeTicks)
%xlabel('$\textrm{Time}[s]$','interpreter','latex','fontsize',fontsizeLabel)
